...
var asistente = {
    verPerfiles: function(opcion){
      ...
      if(opcion === "nombre") {
        var nuevoArray = perfiles.map(function( ele){
            return ele.usuario
        })
        return nuevoArray
      }
      if ( opcion === "codigo" || opcion === "antiguedad") return perfiles.map(perfil => perfil[opcion])
    ...
    modificarAcceso: function(usuario, codigo){
        /* TU CODIGO */
        var usuarioEncontrado = perfiles.find(ele => {
